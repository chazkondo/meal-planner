...
function updateItemState(item) {
  ...
  return (
    ...
    <div>
        {!loading ? allRecipes.length ? allRecipes.map(item=><div key={item._id} onClick={()=>sanity(item)}>{item.name}</div>): <span>No Recipes. <a onClick={()=>router.push('/createitems')}>Click To Add</a></span> : <span>Loading</span>}
        {/* Edit Item
